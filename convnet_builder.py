...
class ConvNetBuilder(object):
  ...
  def affine(self,
    ...
    with tf.variable_scope(name):
      ...
      if initializers is None:
        initializers=[]
        initializers.append(tf.truncated_normal_initializer(stddev=stddev))
        initializers.append(tf.constant_initializer(bias))
    ...
    with tf.variable_scope(name):
      ...
      if self.use_tf_layers:
        dropout = core_layers.dropout(input_layer, 1. - keep_prob,
                                      training=self.phase_train and self.trainable)
      else:
  ...
  def _batch_norm_without_layers(self, input_layer, decay, use_scale, epsilon):
    ...
    moving_variance = tf.get_variable('moving_variance', [num_channels],
                                      ...
                                      trainable=False)
    if self.phase_train and self.trainable:
    ...
    with tf.variable_scope(name) as scope:
      if self.use_tf_layers:
        bn = tf.contrib.layers.batch_norm(
            ...
            scale=scale,
            epsilon=epsilon,
            is_training=self.phase_train and self.trainable,
            fused=True,
            data_format=self.data_format,
            scope=scope)
      else:
